type StoreName = 'session' | 'users' | 'teams' | 'discussions' | 'comments';
...
> = {
  ...
  users: { keyPath: 'id' },
  teams: { keyPath: 'id' },
  discussions: { keyPath: 'id' },
  comments: {
    keyPath: 'id',
    indexes: [{ name: 'by-discussionId', keyPath: 'discussionId' }],
  },
};
...
export class LocalDatabase {
  ...
  private readonly fallbackPrefix = 'rootstock-local-db';
  ...
  async getAllByIndex<T>(
    ...
    if (!this.hasIndexedDB()) {
      const all = this.fallbackGetAll<any>(store);
      const idx = STORE_CONFIG[store].indexes?.find(
        (i) => i.name === indexName,
      );
      if (!idx) return [];
  ...
  private async doSeed(): Promise<void> {
      ...
      for (const name of STORE_NAMES) {
        if (this.initialData[name].length > 0) {
          localStorage.setItem(
            ...
            JSON.stringify(this.initialData[name]),
          );
        }
      }
      return;
    ...
    const hasInitialData = STORE_NAMES.some(
      (name) => this.initialData[name].length > 0,
    );
