...
export const ShouldOnlyOpenOneWhenSingleType: Story = {
	...
	play: async ({ canvasElement }) => {
		const accordions = await within(canvasElement).getAllByRole('button');

		// Open the tabs one at a time
		for (const trigger of accordions) {
			await userEvent.click(trigger);
			await waitFor(async () => {
				const content = await within(canvasElement).findAllByRole('region');
		...
		await waitFor(async () => {
			const content = await within(canvasElement).queryByRole('region');
