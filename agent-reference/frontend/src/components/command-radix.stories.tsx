...
export const TypingInCombobox: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		expect(
			within(canvasElement).getAllByRole('option', { name: /calendar/i }),
		).toHaveLength(1);
		...
		expect(within(canvasElement).getAllByRole('option').length).toBeGreaterThan(
			1,
		);
		expect(
			within(canvasElement).getAllByRole('option', { name: /search/i }),
		).toHaveLength(1);
		...
		expect(
			within(canvasElement).queryAllByRole('option', { hidden: false }),
		).toHaveLength(0);
		expect(within(canvasElement).getByText(/no results/i)).toBeVisible();
	},
};
