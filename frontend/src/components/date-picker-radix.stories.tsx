					...
					<PopoverContent className="w-auto overflow-hidden p-0" align="start">
						<Calendar
							...
							onSelect={(date) => {
								setDate(date);
								setOpen(false);
								fn().mockName('date selected')(date);
							}}
						/>
					</PopoverContent>
				</Popover>
...
export const ShouldOpenPopover: Story = {
	...
	play: async ({ canvasElement, step }) => {
		...
		await step('Select a date', async () => {
			const dateButtons = await within(canvasElement).findAllByRole('button', {
				name: /1st/i,
			});
					...
					<Input
						...
						onChange={(e) => {
							...
							if (isValidDate(date)) {
								setDate(date);
								setMonth(date);
								fn().mockName('date input changed')(date);
							}
						}}
						onKeyDown={(e) => {
							if (e.key === 'ArrowDown') {
				...
				<div className="flex flex-col gap-3">
					...
					<Input
						...
						onChange={(e) => {
							...
							setDate(date);
							fn().mockName('time selected')(date);
						}}
...
export const ShouldOpenCalendar: Story = {
	...
	play: async ({ canvasElement, step }) => {
		await step('Open the date popover', async () => {
			const dateInput = await within(canvasElement).findByLabelText('Date');
			await userEvent.click(dateInput);
			await waitFor(() =>
				expect(
					within(canvasElement)
						.queryAllByRole('button', { name: /1st/i })
						.at(0),
				).toBeVisible(),
			);
		});
		...
		await step('type a time', async () => {
			const timeInput = await within(canvasElement).findByLabelText('Time');
