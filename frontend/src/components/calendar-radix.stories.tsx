...
const meta = {
	...
	args: {
		mode: 'single',
		selected: new Date(),
		onSelect: fn().mockName('onDayClick'),
...
export const ShouldNavigateMonthsWhenClicked: Story = {
	...
	play: async ({ canvasElement }) => {
		const title = await within(canvasElement).findByText(/2000/i);
		const startTitle = title.textContent || '';
		const backBtn = await within(canvasElement).findByRole('button', {
			name: /previous/i,
		});
		const nextBtn = await within(canvasElement).findByRole('button', {
			name: /next/i,
		});
		const steps = 6;
